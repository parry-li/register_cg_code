...
//        if(mSelectedCollection.getCollectionType()>=1){
//            return;
//        }
        if (mSelectionSpec.countable) {
            int mSize =   mSelectedCollection.count();
            if(mSize>=1){
                List<Item> selectList = mSelectedCollection.asList();
                for(Item it :selectList){
                    if (mSelectedCollection.isSelected(it)) {
                        mSelectedCollection.remove(it);
                        notifyCheckStateChanged();
                    } else {
                        if (assertAddSelection(holder.itemView.getContext(), it)) {
